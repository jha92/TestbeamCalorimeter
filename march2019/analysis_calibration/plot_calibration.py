...
# DATA
energy = np.loadtxt('fitparameter.txt', delimiter=',', usecols=(0), dtype=str, unpack=True)
energy = [ float(it.replace('GeV','.0')) for it in energy]
volt,sigma = np.loadtxt('fitparameter.txt', delimiter=',', usecols=(1,2), unpack=True)
energy_spread = 0.158
...
print('fit result: p0=%.4f+-%.4f; p1=%.4f+-%.4f'%(para[0],perr[0],para[1],perr[1]))
