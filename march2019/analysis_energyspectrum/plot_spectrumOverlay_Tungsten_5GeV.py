...
#0.1mm
with open('../data/190308_165000.csv','r') as infile:
...
#0.2mm
with open('../data/190308_154941.csv','r') as infile:
    data = infile.readlines()[1:-1]
bins  = np.array([float(row.split(",")[0]) for row in data])
counts = np.array([float(row.split(",")[1]) for row in data])
bins = bins * -1.
...
bins = bins - binning/2
counts = counts / 637
bins_0pt2mm = calibfunc(bins)
max_0pt2mm = np.where(np.max(counts[5:])==counts)[0][0]
counts_0pt2mm = counts / np.sum(counts)

#1mm
with open('../data/190308_151031.csv','r') as infile:
    data = infile.readlines()[1:-1]
bins  = np.array([float(row.split(",")[0]) for row in data])
counts = np.array([float(row.split(",")[1]) for row in data])
bins = bins * -1.
...
#2mm
with open('../data/190308_152907.csv','r') as infile:
    data = infile.readlines()[1:-1]
bins  = np.array([float(row.split(",")[0]) for row in data])
counts = np.array([float(row.split(",")[1]) for row in data])
bins = bins * -1.
...
bins = bins - binning/2
counts = counts / 637
bins_2mm = calibfunc(bins)
max_2mm = np.where(np.max(counts[5:])==counts)[0][0]
counts_2mm = counts / np.sum(counts)

#5mm
with open('../data/190308_135342.csv','r') as infile:
    data = infile.readlines()[1:-1]
bins  = np.array([float(row.split(",")[0]) for row in data])
counts = np.array([float(row.split(",")[1]) for row in data])
bins = bins * -1.
...
plt.bar(bins_0pt2mm, counts_0pt2mm, width=binning, color='yellowgreen', label='0.2mm', alpha=0.5)
plt.bar(bins_1mm,    counts_1mm,    width=binning, color='lawngreen',   label='1mm',   alpha=0.5)
plt.bar(bins_2mm,    counts_2mm,    width=binning, color='lightgreen',  label='2mm',   alpha=0.5)
...
plt.step(bins_0pt2mm, counts_0pt2mm, color='yellowgreen', linewidth='1', label='0.2mm')
plt.step(bins_1mm   , counts_1mm,    color='lawngreen',   linewidth='1', label='1mm')
plt.step(bins_2mm   , counts_2mm,    color='lightgreen',  linewidth='1', label='2mm')
