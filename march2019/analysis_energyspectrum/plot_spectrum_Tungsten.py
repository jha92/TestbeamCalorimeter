...
# INPUT
name = 'Tungsten'
data_inputs = ['../data/190308_165000.csv','../data/190308_154941.csv','../data/190308_151031.csv','../data/190308_152907.csv','../data/190308_135342.csv',
		'../data/190308_170905.csv','../data/190308_172713.csv','../data/190308_174439.csv']
beam_energies = ['5GeV','5GeV','5GeV','5GeV','5GeV','2GeV','2GeV','2GeV']
thickness_names = ['0pt1mm','0pt2mm','1mm','2mm','5mm','0pt1mm','1mm','5mm']
thickness_titles = ['0.1mm','0.2mm','1mm','2mm','5mm','0.1mm','1mm','5mm']
colors = ['olive','yellowgreen','lawngreen','lightgreen','green','olive','lawngreen','green']
...
for i in range(len(data_inputs)):
	...
	plt.bar(bins, counts, width=binning, color=colors[i], label='%s %s @ %s'%(name,beam_energies[i],thickness_titles[i]))
	plt.yscale('log')
	plt.ylim(1., 1e4)
	...
	plt.bar(bins, counts_norm, width=binning, color=colors[i], label='%s %s @ %s'%(name,beam_energies[i],thickness_titles[i]))
	plt.yscale('log')
	plt.ylim(1e-5, 3e-1)
	plt.xlabel(r'energy [GeV]')
	...
	fig, ax = plt.subplots(figsize=(4, 3), dpi=300)
	fig.subplots_adjust(left=0.17, right=0.95, top=0.95, bottom=0.17)
	bar = plt.bar(bins, counts, width=binning, color=colors[i], label='%s %s @ %s'%(name,beam_energies[i],thickness_titles[i]))
